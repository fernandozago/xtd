...
static std::string serialize(const log_message& message) {
    ...
    const auto append_attribute = [&](std::string_view key, std::string_view value_json) {
        if (!first_attribute) {
            json += ',';
        }
        first_attribute = false;
        json += std::format("{{\"key\":\"{}\",\"value\":{}}}", key, value_json);
    };

    append_attribute("message.template",
        std::format("{{\"stringValue\":\"{}\"}}", escape_json(message.format())));

    const auto formatted_args = message.get_formatted_args();
    for (std::size_t index = 0; index < formatted_args.size(); ++index) {
        append_attribute(std::format("arg{}", index),
            std::format("{{\"stringValue\":\"{}\"}}", escape_json(formatted_args[index])));
    }

    append_attribute("code.file.path",
        std::format("{{\"stringValue\":\"{}\"}}", escape_json(message.location().file_name())));
    append_attribute("code.line.number",
        std::format("{{\"intValue\":{}}}", message.location().line()));
    append_attribute("code.function.name",
        std::format("{{\"stringValue\":\"{}\"}}", escape_json(message.location().function_name())));
